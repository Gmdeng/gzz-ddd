        ...
        return new SQL() {
            {
                ...
                if (!StringUtil.isNull(zModule.getParentId())) {
                    VALUES("parent_id", "#{parentId, jdbcType=BIGINT}");
                }
                if (!StringUtil.isNull(zModule.getType())) {
                    VALUES("type", "#{type, jdbcType=CHAR}");
                }
                if (!StringUtil.isNull(zModule.getName())) {
                    VALUES("name", "#{name, jdbcType=VARCHAR}");
                }
                if (!StringUtil.isNull(zModule.getCode())) {
                    VALUES("code", "#{code, jdbcType=VARCHAR}");
                }
                if (!StringUtil.isNull(zModule.getOperate())) {
                    VALUES("operate", "#{operate, jdbcType=TINYINT}");
                }
                if (!StringUtil.isNull(zModule.getIcon())) {
                    VALUES("icon", "#{icon, jdbcType=VARCHAR}");
                }
                if (!StringUtil.isNull(zModule.getUrl())) {
                    VALUES("url", "#{url, jdbcType=VARCHAR}");
                }
                if (!StringUtil.isNull(zModule.getIdx())) {
                    VALUES("idx", "#{idx, jdbcType=TINYINT}");
                }
                if (!StringUtil.isNull(zModule.getStatus())) {
                    VALUES("status", "#{status, jdbcType=TINYINT}");
                }
                if (!StringUtil.isNull(zModule.getUpdateOn())) {
                    VALUES("update_on", "#{updateOn, jdbcType=TIMESTAMP}");
                }
                if (!StringUtil.isNull(zModule.getUpdateBy())) {
                    VALUES("update_by", "#{updateBy, jdbcType=VARCHAR}");
                }
                if (!StringUtil.isNull(zModule.getCreateOn())) {
                    VALUES("create_on", "#{createOn, jdbcType=TIMESTAMP}");
                }
                if (!StringUtil.isNull(zModule.getCreateBy())) {
        ...
        return new SQL() {
            {
                ...
                if (!StringUtil.isNull(zModule.getOperate())) {
                    SET("operate=#{operate,jdbcType=TINYINT}");
                }
                if (!StringUtil.isNull(zModule.getIcon())) {
                ...
                if (!StringUtil.isNull(zModule.getUpdateOn())) {
                    SET("update_on=#{updateOn,jdbcType=TIMESTAMP}");
                }
                if (!StringUtil.isNull(zModule.getUpdateBy())) {
